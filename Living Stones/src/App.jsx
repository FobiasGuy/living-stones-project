...
async function loadOrders() {
  ...
  return toDict(data, (o) => ({
    ...
    items: (o.order_items || []).map((it) => ({ productId: it.product_id, name: it.name, price: it.price, qty: it.qty })),
...
function AdminOrders({ orders, onUpdate, onStageChange }) {
  const [open, setOpen] = useState(null);
...
function AdminOrderDetail({ order, onUpdate, onStageChange }) {
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-stone-100">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4">
          ...
          {tab === "orders" && <AdminOrders orders={data.orders} onUpdate={actions.updateOrder} onStageChange={actions.updateOrderStage} />}
    ...
    return (
      <div className="min-h-screen bg-stone-100" style={{ colorScheme: "light" }}>
        ...
        <AdminManager
          ...
          actions={{ saveProduct, deleteProduct, updateOrder, updateOrderStage, updateRequest, saveThing, deleteThing, setRole, saveSettings }}
