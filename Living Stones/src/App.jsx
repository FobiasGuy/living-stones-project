...
import {
  Search, ShoppingCart, User, Package, MessageSquare, MessageCircle, Home as HomeIcon,
...
async function loadOrders() {
  ...
  return toDict(data, (o) => ({
    ...
    studentClass: o.class || "", paymentMethod: o.payment_method || "",
...
/* Chat: one thread per customer, shared by every admin/owner (sender_role
   tells you which "side" of the conversation a message came from). */

async function loadChatMessages(customerUsername) {
  const { data } = await supabase
    .from("chat_messages")
    .select("*")
  ...
  return (data || []).map((m) => ({
    id: m.id, customerUsername: m.customer_username, senderUsername: m.sender_username,
    senderRole: m.sender_role, body: m.body, createdAt: m.created_at,
  }));
}
...
async function loadChatThreads() {
  ...
  const threads = {};
  (data || []).forEach((m) => {
    if (!threads[m.customer_username]) {
      threads[m.customer_username] = { customerUsername: m.customer_username, lastBody: m.body, lastAt: m.created_at, lastSenderRole: m.sender_role };
    }
  });
  return Object.values(threads).sort((a, b) => new Date(b.lastAt) - new Date(a.lastAt));
}

async function sendChatMessage(customerUsername, senderUsername, senderRole, body) {
  const trimmed = (body || "").trim();
  if (!trimmed) return { error: null };
  const { error } = await supabase.from("chat_messages").insert({
    customer_username: customerUsername, sender_username: senderUsername, sender_role: senderRole, body: trimmed,
  });
  return { error };
}
...
function CartDrawer({ open, onClose, cart, products, onQty, onRemove, onCheckout, currentUser }) {
  ...
  // reset the checkout fields every time the drawer is (re)opened
  useEffect(() => {
    if (open) {
      setStudentClass("");
  ...
  function handleCheckoutClick() {
    if (!currentUser) { onCheckout(); return; } // lets root's placeOrder() open the login modal
    if (!studentClass) { setCheckoutError("Please select your class."); return; }
    setCheckoutError("");
    onCheckout(studentClass, paymentMethod);
  }
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3 border-t border-stone-200 pt-3">
            <Select label="Your class" value={studentClass} onChange={(e) => setStudentClass(e.target.value)}>
              <option value="">Select your class…</option>
              {CLASS_OPTIONS.map((c) => <option key={c} value={c}>{c}</option>)}
            </Select>

            <div>
              <span className="ls-body mb-1 block text-xs font-medium text-stone-500">Payment method</span>
              <div className="flex gap-2">
                {PAYMENT_METHODS.map((m) => (
                  <button
                    ...
                    className={`ls-body flex-1 rounded-xl border px-3 py-2 text-sm font-medium transition-colors ${
                      paymentMethod === m
              ...
              {paymentMethod === "Transfer" && (
                <p className="ls-body mt-1.5 text-[11px] text-stone-400">
                  You'll get transfer details after placing your order.
                </p>
              )}
            </div>
          ...
          <Btn className="w-full" onClick={handleCheckoutClick}>
      ...
      <Modal open={!!open} onClose={() => setOpen(null)} title={open?.id || ""} wide>
        {open && (
          <div className="space-y-4">
            ...
            <p className="ls-body text-xs text-stone-500">Payment method: <span className="font-medium text-stone-700">{open.paymentMethod}</span></p>
            <p className="ls-body text-xs text-stone-500">Class: <span className="font-medium text-stone-700">{open.studentClass}</span></p>
...
function ChatBubble({ message, mine }) {
  return (
    <div className={`flex ${mine ? "justify-end" : "justify-start"}`}>
      <div className={`ls-body max-w-[75%] rounded-2xl px-3 py-2 text-sm ${mine ? "bg-stone-800 text-stone-50" : "border border-stone-200 bg-stone-100 text-stone-700"}`}>
        {!mine && <p className="ls-body mb-0.5 text-[10px] font-semibold uppercase tracking-wide opacity-60">{message.senderUsername}</p>}
        <p className="whitespace-pre-line">{message.body}</p>
        <p className="ls-body mt-1 text-[10px] opacity-60">{formatDate(message.createdAt)}</p>
      </div>
    </div>
  );
}

function ChatView({ currentUser, onOpenAuth }) {
  ...
  useEffect(() => {
    ...
    async function refresh() {
      const msgs = await loadChatMessages(currentUser.username);
      if (!cancelled) { setMessages(msgs); setLoading(false); }
    }
    refresh();
    const interval = setInterval(refresh, 4000); // simple polling — no realtime subscription needed at this scale
  ...
  async function handleSend(e) {
    ...
    const body = text;
    setText("");
    const { error } = await sendChatMessage(currentUser.username, currentUser.username, "customer", body);
    if (!error) setMessages(await loadChatMessages(currentUser.username));
  }

  if (!currentUser) {
    return (
      <div className="space-y-4">
        <h1 className="ls-display text-xl font-bold text-stone-800">Chat</h1>
        <EmptyState icon={MessageCircle} title="Log in to chat with us" sub="Ask about an order, a custom request, or anything else." />
        <Btn onClick={onOpenAuth} className="mx-auto">Log in</Btn>
      </div>
    );
  }

  return (
    <div className="flex h-[calc(100vh-230px)] min-h-[380px] flex-col">
      <h1 className="ls-display mb-3 text-xl font-bold text-stone-800">Chat with us</h1>
      <div className="flex-1 space-y-2 overflow-y-auto rounded-2xl border border-stone-200 bg-white p-3">
        {loading ? (
          <div className="flex h-full items-center justify-center text-stone-400"><Loader2 className="animate-spin" size={18} /></div>
        ) : messages.length === 0 ? (
          <EmptyState icon={MessageCircle} title="No messages yet" sub="Say hi — an admin or the owner will see it here." />
        ) : (
          messages.map((m) => <ChatBubble key={m.id} message={m} mine={m.senderRole === "customer"} />)
        )}
      </div>
      <form onSubmit={handleSend} className="mt-3 flex gap-2">
        <input
          ...
          className="ls-body flex-1 rounded-full border border-stone-300 bg-white px-4 py-2 text-sm outline-none focus:border-stone-500 focus:ring-2 focus:ring-stone-200"
        />
        ...
        <div className="space-y-2">
          {list.map((o) => (
            <button key={o.id} onClick={() => setOpen(o)} className="w-full rounded-xl border border-stone-200 bg-white p-3 text-left hover:bg-stone-50">
              <div className="flex items-center justify-between">
                <span className="ls-mono text-xs text-stone-500">{o.id}</span>
                <span className="ls-body text-xs text-stone-400">{o.username}{o.studentClass ? ` · ${o.studentClass}` : ""}</span>
              </div>
              <div className="mt-1 flex items-center justify-between">
                <span className="ls-body text-xs font-medium text-stone-600">{o.stage}</span>
                <span className="ls-body text-xs text-stone-500">{o.paymentStatus} · {o.paymentMethod}</span>
              </div>
            </button>
          ))}
        </div>
...
function AdminOrderDetail({ order, onUpdate, onStageChange, onDelete }) {
  ...
  return (
    <div className="space-y-4">
      <div className="ls-body grid grid-cols-2 gap-2 rounded-xl bg-stone-50 p-3 text-xs text-stone-500">
        <span>Class: <span className="font-medium text-stone-700">{order.studentClass || "—"}</span></span>
        <span>Payment method: <span className="font-medium text-stone-700">{order.paymentMethod || "—"}</span></span>
      </div>
      <div className="space-y-1.5">
...
function AdminMessageThread({ customerUsername, currentUser }) {
  const [messages, setMessages] = useState([]);
  const [text, setText] = useState("");
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    let cancelled = false;
    async function refresh() {
      const msgs = await loadChatMessages(customerUsername);
      if (!cancelled) { setMessages(msgs); setLoading(false); }
    }
    refresh();
  ...
  async function handleSend(e) {
    ...
    const body = text;
    setText("");
    const { error } = await sendChatMessage(customerUsername, currentUser.username, "staff", body);
    if (!error) setMessages(await loadChatMessages(customerUsername));
  }

  return (
    <div className="flex h-[60vh] flex-col">
      <div className="flex-1 space-y-2 overflow-y-auto rounded-xl border border-stone-200 bg-stone-50 p-3">
        {loading ? (
          <div className="flex h-full items-center justify-center text-stone-400"><Loader2 className="animate-spin" size={18} /></div>
        ) : messages.length === 0 ? (
          <EmptyState icon={MessageCircle} title="No messages yet" />
        ) : (
          messages.map((m) => <ChatBubble key={m.id} message={m} mine={m.senderRole === "staff"} />)
        )}
      </div>
      <form onSubmit={handleSend} className="mt-3 flex gap-2">
        <input
...
function AdminMessages({ currentUser }) {
  ...
  useEffect(() => {
    let cancelled = false;
    async function refresh() {
      const t = await loadChatThreads();
      if (!cancelled) { setThreads(t); setLoading(false); }
    }
    refresh();
  ...
  return (
    <div className="space-y-3">
      <h2 className="ls-display text-sm font-semibold text-stone-700">Messages</h2>
      {loading ? (
        <div className="flex justify-center py-10 text-stone-400"><Loader2 className="animate-spin" size={18} /></div>
      ) : threads.length === 0 ? (
        <EmptyState icon={MessageCircle} title="No conversations yet" sub="Messages from customers will show up here." />
      ) : (
        <div className="space-y-2">
          {threads.map((t) => (
            <button key={t.customerUsername} onClick={() => setOpenThread(t.customerUsername)} className="flex w-full items-center justify-between gap-2 rounded-xl border border-stone-200 bg-white p-3 text-left hover:bg-stone-50">
              <div className="min-w-0">
                <p className="ls-body text-sm font-medium text-stone-700">{t.customerUsername}</p>
                <p className="ls-body truncate text-xs text-stone-400">{t.lastSenderRole === "staff" ? "You: " : ""}{t.lastBody}</p>
              </div>
              <span className="ls-body shrink-0 text-[10px] text-stone-400">{formatDate(t.lastAt)}</span>
            </button>
          ))}
        </div>
      )}
      <Modal open={!!openThread} onClose={() => setOpenThread(null)} title={openThread || ""} wide>
        {openThread && <AdminMessageThread customerUsername={openThread} currentUser={currentUser} />}
      </Modal>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-stone-100">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4">
          ...
          {tab === "messages" && <AdminMessages currentUser={currentUser} />}
...
export default function LivingStonesProject() {
  ...
  async function placeOrder(studentClass, paymentMethod) {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <main className="mx-auto max-w-5xl px-4 py-6">
        ...
        {view === "chat" && <ChatView currentUser={currentUser} onOpenAuth={() => setAuthOpen(true)} />}
      </main>
